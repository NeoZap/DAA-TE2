...
def BnB(G, T):
    ...
    while Frontier != [] and delta_time < T:
        ...
        if CurG.number_of_edges() == 0:  # end of exploring, solution found
            ...
            if CurVC_size < UpperBound:
                ...
                metrics.append((CurVC_size, time.time() - start_time, peak_mem_usage))
            backtrack = True
    ...
    tracemalloc.stop()
    return OptVC, metrics


# TO FIND THE VERTEX WITH MAXIMUM DEGREE IN REMAINING GRAPH
def find_maxdeg(g):
    deglist = dict(g.degree())  # Convert DegreeView to a dictionary
    # Sort dictionary by degree
    deglist_sorted = sorted(deglist.items(), reverse=True, key=lambda x: x[1])
...
def main(inputfile, output_dir, cutoff, randSeed):
    ...
    # WRITE SOL FILES
    with open(
        ".\Output\\" + inputfile.split(".")[0] + "_BnB_" + str(cutoff) + ".sol", "w"
    ) as f:
        f.write("%i\n" % (len(Sol_VC)))
    ...
    # WRITE TRACE FILES
    with open(
        ".\Output\\" + inputfile.split(".")[0] + "_BnB_" + str(cutoff) + ".trace", "w"
    ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-inst", action="store", type=str, required=True, help="Inputgraph datafile"
    )
    parser.add_argument(
        "-alg",
        action="store",
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="Cutoff running time for algorithm",
    )
    parser.add_argument(
        "-seed",
        action="store",
