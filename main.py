...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Wrapper Function for Vertex Cover Graphs"
    )
    parser.add_argument(
        "-inst", action="store", type=str, required=True, help="Input graph datafile"
    )
    parser.add_argument(
        "-alg",
        action="store",
        type=str,
        required=True,
        help="Input Alg  [BnB|Approx|LS1|LS2]",
    )
    parser.add_argument(
        "-time",
        action="store",
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="Random Seed for algorithm",
    )
    ...
    if algorithm == "Approx":
        ...
        print(
            "Inputted incorrect algorithm option, please try any of: [BnB|Approx|LS1|LS2]"
        )
